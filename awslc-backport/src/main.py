...
Layer: entrypoint. Wires the four command modules (analyze / apply / ci /
resolve) into one argument parser and dispatches to them. See the module map at
the end of this docstring for how the rest of the package fits together.
...
Subcommands:
  ...
  resolve [--pr N | --commit <sha>]
                      Interactive, local conflict resolution. Reads the backport
                      bot's already-decided plan from the PR (or the last analyze
                      run), so it needs no AI -- it just fixes the branches that
                      conflicted. For each, it checks the branch out in your
                      CURRENT repo (in-place, the default) with the conflict live,
                      you edit and exit, and git rerere reuses a resolution across
                      sibling branches. When done, optionally opens one normal PR
                      per branch. Fork remotes only.

                      Common case is just:  backport resolve --pr N
                      (--repo defaults to the current directory; --ai re-enables
                      the AI on the --reanalyze path; --worktree isolates instead
                      of editing in place).
...
def add_resolve(sub) -> None:
    ...
    p.add_argument(
        ...
        "backport bot's plan from the PR (re-runs the impact analysis)",
    )
    p.add_argument(
        "--worktree",
        dest="in_place",
        ...
        "(default: resolve in-place in your current repo so your IDE reflects it)",
    )
    p.add_argument(
        "--remote",
        default="origin",
    ...
    p.add_argument(
        ...
        dest="no_ai",
        action="store_false",
        help="consult the AI when reanalyzing locally (default: off -- resolve "
        "reads the already-decided plan from the PR/last analyze, so AI is not "
        "needed)",
    )
    add_common(p)
